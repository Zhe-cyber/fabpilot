...
from sim.scenario import canned_readings
...
async def run(
    ...
    *,
    ...
    """Run the canned scenario once (sim/scenario.py — shared with the golden test).
    If `emit(kind, data)` is given, stream every step to it — kind is
    "reading" | "anomaly" | "agent" — so a UI (dashboard) can render the same events
    the console prints. `delay` paces readings so a live view streams visibly.
